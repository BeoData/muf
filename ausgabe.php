...
class MM2_Ausgabe extends MM25_Edition
{
    ...
    public array $linkedThemenheftID;
    // public string $term_themen_link;
    public array $term_themen_link;
    //  public string $linkedThemenheftName;
    public array $linkedThemenheftName;
    ...
    function __construct()
    {
        ...
        $this->linkedThemenheftID = [];
        $this->linkedThemenheftName= [];
    ...
    public function MM2_GetAusgabe(int $sAusgabeID, string $current_media , string $stype = 'full' )
    {
        ...
        if (
            ...
            $linked_edition_slugs = explode(',', $this->aSingleOptionAusgabe['ausgabe_meta_themenheft_id']);

            foreach ($linked_edition_slugs as $_linked_edition_slug) {
                $_lmeta_term = get_term_by('slug', trim($_linked_edition_slug), $this->sCurrentAusgabenTaxonomy);


                if (!empty($_lmeta_term)) {
                    // Provera tipa pre dodavanja u niz
                    if (is_array($this->linkedThemenheftName)) {
                        $this->linkedThemenheftName[] = $_lmeta_term->name;
                    } else {
                        $this->linkedThemenheftName = [$_lmeta_term->name];
                    }

                    // Slično i za term_themen_link
                    if (is_array($this->term_themen_link)) {
                        $this->term_themen_link[] = get_term_link($_lmeta_term->term_id, $this->sCurrentAusgabenTaxonomy);
                    } else {
                        $this->term_themen_link = [get_term_link($_lmeta_term->term_id, $this->sCurrentAusgabenTaxonomy)];
                    }
                }
...
//break;
        ...
        if ( $stype == 'full')
        {
            ...
            if (
                ...
                if (count($articles_themenheft_query) > 0) {
                    ...
                    // Ako nema pronađenih članaka, postavite $this->aThemenheftArticles na prazan niz
                    $this->aThemenheftArticles = array();
                }
            }
            // Step 3
            //$this->aThemenheftArticles = MM25_GetEditionArticles( '',);



         //   var_dump($this->aThemenheftArticles );
         //  die();
            // Step 4
            // Content Ads
    ...
    public function MM2_GetAusgabeMeta(int $sAusgabeID, string $current_media )
    {
        ...
        // Initialize the arrays if they haven't been initialized yet
        if (!is_array($this->linkedThemenheftID)) {
            $this->linkedThemenheftID = [];
        }
        if (!is_array($this->linkedThemenheftName)) {
            $this->linkedThemenheftName = [];
...
// Themenheft Linkage IF not a themenheft, but a Themenhaft links to this Edition
        if ($meta_term_slug && !$this->bIsThemenheft)
        {
            foreach ($this->aOptionsAusgaben as $ID => $value)
            {
                if (empty($value))
...
function EchoEditionMeta( $Edition)
{
    ...
    */
    ...
    // Initialize an empty array to keep track of unique names
    $uniqueNames = [];

// Themenheft Anchor Link
    if ($Edition->term_themen_link && is_array($Edition->term_themen_link))
    {
        // Themenheft Desc
        if ($Edition->sThemenheftDesc)
        {
            echo('<p>' . $Edition->sThemenheftDesc . '</p>');
        }

        foreach ($Edition->term_themen_link as $key => $link)
        {
            $name = $Edition->linkedThemenheftName[$key];

            // Skip if the name is already displayed
            ...
            if (!$Edition->bIsThemenheft)
            {
                $themenheftID = $Edition->linkedThemenheftID[$key];
                echo "<a href=\"#themenheft_{$themenheftID}\">{$name}</a><br>";
            }
            else
            {
