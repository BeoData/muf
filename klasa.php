class MM2_Ausgabe extends MM25_Edition
{
	...
	public function MM2_GetAusgabe(int $sAusgabeID, string $current_media , string $stype = 'full' )
	{
        ...
        $ausgabe_meta_themenheft_id = $this->aSingleOptionAusgabe['ausgabe_meta_themenheft_id'];
        var_dump($ausgabe_meta_themenheft_id);
        ...
        if (
            ...
            $_linked_edition_slugs = explode(',', $this->aSingleOptionAusgabe['ausgabe_meta_themenheft_id']);

            foreach ($_linked_edition_slugs as $_linked_edition_slug) {
		...
		)
		{
			foreach ( $this->aOptionsAusgaben as $ID => $value)
			{
				...
				)
				{
                  ...
                  // $this->linkedThemenheftID = $ID; // Convert this to a keyed array
